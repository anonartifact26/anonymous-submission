# -*- coding: utf-8 -*-
"""
Main experiment: Adult + Bank Marketing
Enhanced full-visualization version for stationary arm-resolved homogeneous batching

What this script does:
- builds stationary unimodal environments from real tabular datasets
- runs 30 seeds for:
    * UCB
    * KL-UCB / line-graph OSUB
- saves long-form results
- generates multiple plots:
    1) cumulative realized regret curves
    2) final realized regret boxplots
    3) mean final regret heatmap
    4) KL-vs-UCB win-rate heatmap
    5) arm-pull heatmaps
    6) optimal-neighborhood concentration heatmap
    7) paired scatter plots
    8) arm trajectory plots
    9) relative improvement bars
   10) environment profile plots
   11) final realized regret violin plots
   12) summary tables with mean/std/median/95% CI/bootstrap CI
   13) paired significance tests (paired t-test / Wilcoxon)
...
def plot_regret_curves_and_box(dataset_name: str, df_long: pd.DataFrame):
    ...
    for algo in ALGORITHMS:
        ...
        ax.fill_between(x, mean - ci, mean + ci, color=COLOR_MAP[algo], alpha=0.22)
...
def plot_paired_scatter(dataset_name: str, df_final: pd.DataFrame):
    ...
    if not set(ALGORITHMS).issubset(piv.columns):
        return

    x = piv["UCB"].values
    y = piv["KLUCB_OSUB"].values

    m = max(float(np.max(x)), float(np.max(y))) * 1.05

    fig, ax = plt.subplots(figsize=(5.4, 5.2))
    ax.scatter(x, y, s=48, color="#8c564b", alpha=0.85, edgecolor="black", linewidth=0.5)
    ax.plot([0, m], [0, m], ls="--", color="gray", lw=1.5)

    better = int(np.sum(y < x))
    equal = int(np.sum(y == x))
    worse = int(np.sum(y > x))

    ax.set_title(f"{dataset_name}: paired final regret\nKL-UCB better on {better}/30 seeds")
    ax.set_xlabel("UCB final realized regret")
    ax.set_ylabel("KL-UCB final realized regret")
    ax.grid(True, alpha=0.3)
...
def plot_arm_trajectory(dataset_name: str, df_long: pd.DataFrame, env: StationaryBatchedEnv):
    ...
    for algo in ALGORITHMS:
        ...
        ax.fill_between(x, mean - std, mean + std, color=COLOR_MAP[algo], alpha=0.18)
...
def plot_relative_improvement(df_final: pd.DataFrame):
    ...
    for dataset in DATASET_ORDER:
        ...
        rows.append({"dataset": dataset, "improvement_pct": improve})

    sdf = pd.DataFrame(rows)

    fig, ax = plt.subplots(figsize=(6.8, 4.6))
    bars = ax.bar(sdf["dataset"], sdf["improvement_pct"], color="#17becf", alpha=0.8)

    for b in bars:
        h = b.get_height()
        ax.text(b.get_x() + b.get_width()/2, h, f"{h:.1f}%", ha="center", va="bottom")

    ax.axhline(0, color="black", lw=1)
    ax.set_title("Relative improvement of KL-UCB over UCB")
...
def plot_global_heatmaps(df_final: pd.DataFrame, df_neigh: pd.DataFrame):
    ...
    for i, ds in enumerate(DATASET_ORDER):
        ...
        win_rate = np.mean(piv["KLUCB_OSUB"].values < piv["UCB"].values)
        mat3[i, 0] = float(win_rate)
...
def build_summary_tables(
    ...
    for (dataset, algo), sub in df_final.groupby(["dataset", "algorithm"]):
        ...
        bci_low_r, bci_high_r = bootstrap_ci_mean(
            vals, B=BOOTSTRAP_B, alpha=0.05, seed=BOOTSTRAP_RANDOM_SEED
        )
        ...
        bci_low_p, bci_high_p = bootstrap_ci_mean(
            pvals, B=BOOTSTRAP_B, alpha=0.05, seed=BOOTSTRAP_RANDOM_SEED
        )
    ...
    for dataset, sub in df_final.groupby("dataset"):
        pivot_r = sub.pivot(index="seed", columns="algorithm", values="final_realized_regret")
        pivot_p = sub.pivot(index="seed", columns="algorithm", values="final_pseudo_regret")

        if set(ALGORITHMS).issubset(set(pivot_r.columns)):
            a = pivot_r["UCB"].values.astype(float)
            b = pivot_r["KLUCB_OSUB"].values.astype(float)
            diff = a - b

            diff_mean = float(np.mean(diff))
            diff_std = float(np.std(diff, ddof=1)) if len(diff) > 1 else 0.0
            diff_med = float(np.median(diff))
            diff_ci_low, diff_ci_high = t_ci_mean(diff, alpha=0.05)
            diff_boot_low, diff_boot_high = bootstrap_ci_mean(
                diff, B=BOOTSTRAP_B, alpha=0.05, seed=BOOTSTRAP_RANDOM_SEED
            )
            ...
            pair_rows.append({
                ...
                "t_ci95_low_diff_A_minus_B": diff_ci_low,
                "t_ci95_high_diff_A_minus_B": diff_ci_high,
                "bootstrap_ci95_low_diff_A_minus_B": diff_boot_low,
                "bootstrap_ci95_high_diff_A_minus_B": diff_boot_high,
                "A_better_count": int(np.sum(a < b)),
                "B_better_count": int(np.sum(b < a)),
                "ties": int(np.sum(a == b)),
            ...
            sig_rows.append({
                ...
                "mean_diff_A_minus_B": diff_mean,
                "median_diff_A_minus_B": diff_med,
        ...
        if set(ALGORITHMS).issubset(set(pivot_p.columns)):
            a = pivot_p["UCB"].values.astype(float)
            b = pivot_p["KLUCB_OSUB"].values.astype(float)
            diff = a - b
    ...
    for dataset in DATASET_ORDER:
        env = env_map[dataset]
        sub = df_final[df_final["dataset"] == dataset].pivot(index="seed", columns="algorithm", values="final_realized_regret")
        ratio = np.mean(sub["KLUCB_OSUB"].values) / max(np.mean(sub["UCB"].values), 1e-12)

        suspicion_rows.append({
            "dataset": dataset,
            "opt_arm": int(env.opt_arm),
            "opt_mean": float(env.opt_mean),
            "utility_gap_best_minus_second": float(
                np.sort(env.mean_rewards)[-1] - np.sort(env.mean_rewards)[-2]
            ) if env.K >= 2 else 0.0,
            "all_structure_assumptions_hold": bool(
                monotone_nondecreasing(env.impacts)
                and monotone_nonincreasing(env.success_probs)
                and is_unimodal(env.mean_rewards)[0]
            ),
            "mean_regret_ratio_KL_over_UCB": float(ratio),
